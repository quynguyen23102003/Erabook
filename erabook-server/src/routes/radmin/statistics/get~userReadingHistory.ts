...
export default async (c: Context) =>
	SessionManager.validateAccessTokenThenExecute(c, async (user) => {
		...
		if (queryBookId && queryBookId.length === 24) {
			condition["shelf.bookId"] = new ObjectId(queryBookId);
		}
