		...
		const condition: Document[] = [
					...
					pipeline: [
						{
							$match: {
								$expr: {
									$or: (user.shelf || []).map((state) => ({
										$eq: [state.bookId, { $toString: "$_id" }],
									})),
								},
							},
						},
			...
			},
			{
				$set: {
					suggestingGenres: {
						// @pref
						// https://zzzcode.ai/code-generator?id=d3a581ba-450e-4536-9f39-a8044d469307
						$reduce: {
							input: { $concatArrays: "$suggestingGenres._genreIds" },
							initialValue: [],
				...
				$match: {
					$expr: {
						$cond: {
							if: {
								$or: [
									{ $gt: [{ $size: "$suggestingGenres" }, 0] },
									{ $gt: [user.preferredGenres?.length ?? 0, 0] },
								],
							},
							then: {
								$or: [
									...
									{ $in: ["$_genreIds", "$suggestingGenres"] },
									// @pref
									// https://zzzcode.ai/code-generator?id=185812d5-e121-466c-b834-aefaeb70744f
									{
										$gt: [
											{
												$size: {
													$setIntersection: ["$_genreIds", "$suggestingGenres"],
												},
											},
											0,
										],
