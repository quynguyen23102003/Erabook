...
export default async (c: Context) =>
	SessionManager.validateAccessTokenThenExecute(c, async (user) => {
		...
		const condition: Document[] = [
			{
				$match: {
					$expr: {
						$in: [
							{ $toString: "$_id" },
							user.shelf?.map((b) => b.bookId.toString()) ?? [],
						],
					},
				},
			},
		...
		switch (querySort) {
			case "title": {
				condition.unshift({
					$sort: { title: querySortOrder, "_user.readingState.timestamp": -1 },
				});
				break;
			}
			case "price": {
				condition.unshift({
					$sort: { price: querySortOrder, "_user.readingState.timestamp": -1 },
				});
				break;
			}
			case "lastUpdated": {
				condition.unshift({
					$sort: {
						...
						"_user.readingState.timestamp": -1,
					},
				});
				break;
			}
			...
			default: {
				condition.unshift({
					$sort: { "_user.readingState.timestamp": -1 },
				});
			}
		}
		...
		// track
		await ActivityLogManager.appendAction(user, null, "search.recent", {
			query: querySearch,
			page: queryPage,
			genre: queryGenre,
			sortBy: querySort,
