	...
	SessionManager.validateAccessTokenThenExecute(c, async (user) => {
		...
		if (taskUpdate.acknowledged) {
			return taskUpdate.modifiedCount
				? c.json({
						status: 200,
						message: "Successfully removed rating from user's favorite list",
				  })
				: c.json(
						{
							status: 400,
							message: "Rating is not in user's favorite list",
						},
						400,
				  );
		}
		...
		return c.json(
			{
				status: 500,
				message: "Failed to remove rating from user's rating list",
			},
			500,
		);
	});
